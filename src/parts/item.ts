...
export class Item extends MyDisplay {
  ...
  constructor(opt:any) {
    ...
    let col = new Color(0x000000).offsetHSL(Util.random(0, 1), 1, 0.5)
    let lineCol = col.clone().offsetHSL(0, 0, -0.35)
    const borderSize = 1
    Tween.set(this._bg, {
      backgroundColor: col.getStyle(),
      border: borderSize + 'px solid ' + lineCol.getStyle(),
    })
    col = col.offsetHSL(0, 0, -0.25)
  ...
  private _getPathStr(isBg: boolean): string {
    ...
    if(isBg) {
      ...
      d += 'M ' + (p4.x) + ' ' + (p4.y) + ' '
      const offset = Util.map(Easing.instance.outExpo(this._rate.val), Util.map(this._noise, 0.1, 0.5, 0, 1), 0, 0.25, 1) * 1
  ...
  protected _update():void {
    ...
    const w = Util.map(Math.sin(this._c * -0.03), 0.1, 30, -1, 1)
    // const h = Util.map(Math.sin(this._c * 0.043), 20, 50, -1, 1)
    Tween.set(this.el, {
      width: w + 'vw',
      height: w + 'vw',
    })
